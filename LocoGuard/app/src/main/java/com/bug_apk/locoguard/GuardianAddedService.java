 ...
 * helper methods.
 */
public class GuardianAddedService extends IntentService {
    ...
    public GuardianAddedService() {
        super("GuardianAddedService");
    }
...
//        Toast.makeText(this, "service starting", Toast.LENGTH_SHORT).show();
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        LocalBroadcastManager.getInstance(GuardianAddedService.this).registerReceiver(stopActivity,
        ...
        initialCheckVEL = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.exists()) {
                    Log.i("GuardianAddedService", "Ward exists");
                    ward = dataSnapshot.getValue(String.class);
                    Intent intent = new Intent("guardAddedBroadcast");
                    intent.putExtra("ward", ward);
                    LocalBroadcastManager.getInstance(GuardianAddedService.this).sendBroadcast(intent);
                    ...
                    database.getReference("journeys").child(ward).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            if(dataSnapshot.exists()) {
                                ...
                                alarmMgr = (AlarmManager)GuardianAddedService.this.getSystemService(Context.ALARM_SERVICE);
                                Intent intent = new Intent(GuardianAddedService.this, ETABroadcastReceiver.class);
                                intent.putExtra("wardName", userJourney.getWardName());
                                Log.i("GuardianAddedService","wardName" + userJourney.getWardName());
                                intent.putExtra("ward", userJourney.getWard());
                                alarmIntent = PendingIntent.getBroadcast(GuardianAddedService.this, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);
                                        ...
                                        if(dataSnapshot.exists() && !routeDeviationFirstTime) {
                                            try {
                                                if (dataSnapshot.getValue(String.class).equals("true")) {
                                                    NotificationCompat.Builder builder = new NotificationCompat.Builder(GuardianAddedService.this, CHANNEL_ID)
                                    ...
                                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                        if(dataSnapshot.exists()) {
                                            ...
                                            if(breakArrayList.get(breakArrayList.size()-1).getBreakDuration()==null) {
                                                alarmMgr.cancel(alarmIntent);
                                                SharedPreferences sharedPref = getSharedPreferences("LocoGuard", Context.MODE_PRIVATE);
                                                if (userDetails.getPreferences().get("journeyPaused").equals("true")) {
                                                    ...
                                                    NotificationManagerCompat notificationManager = NotificationManagerCompat.from(GuardianAddedService.this);
                                                ...
                                                FirebaseDatabase.getInstance().getReference("journeys").child(ward).child("eta").addListenerForSingleValueEvent(new ValueEventListener() {
                                                    ...
                                                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                                        if(dataSnapshot.exists()) {
                                                            ...
                                                            alarmMgr = (AlarmManager) GuardianAddedService.this.getSystemService(Context.ALARM_SERVICE);
                                                            broadcastIntent = new Intent(GuardianAddedService.this, ETABroadcastReceiver.class);
                                                            ...
                                                            alarmIntent = PendingIntent.getBroadcast(GuardianAddedService.this, 0, broadcastIntent, 0);
                                                            SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd' 'HH:mm:ss");
...
//
                                reachedDestinationVEL = new ValueEventListener() {
                                    @Override
                                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                        Log.i("GuardianAddedService", "ReachedDestination exists: " + dataSnapshot.exists());
                                        if(dataSnapshot.exists()) {
                                            Log.i("GuardianAddedService", "ReachedDestination: " + dataSnapshot.getValue(String.class));
                                            if (dataSnapshot.getValue(String.class).equals("true")) {
                                                ...
                                                journeyCompletedVEL = new ValueEventListener() {
                                                    ...
                                                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                                        if(dataSnapshot.exists()) {
                                                            Log.i("GuardianAddedService", "journeyCompleted exists: " + dataSnapshot.exists());
                            ...
                            if(dataSnapshot.exists()) {
                                if(dataSnapshot.getValue(String.class).equals("false")) {
                                    FirebaseDatabase.getInstance().getReference("journeys").child(userDetails.encodedEmail()).addListenerForSingleValueEvent(new ValueEventListener() {
                                        @Override
                                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                            Log.i("GuardianAddedService", "User has a joruney record");
                                            journey journey = dataSnapshot.getValue(journey.class);
                                            Log.i("GuardianAddedService", "User has an on-going joruney. Adding ETA notification service");

                                            alarmMgr = (AlarmManager) GuardianAddedService.this.getSystemService(Context.ALARM_SERVICE);
                                            broadcastIntent = new Intent(GuardianAddedService.this, WardETABroadcastReceiver.class);
        ...
        BroadcastReceiver userLoggedOut = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                Log.i("GuardianAddedService", "User logged out");
        ...
        LocalBroadcastManager.getInstance(GuardianAddedService.this).registerReceiver(userLoggedOut,
                new IntentFilter("LogoutBroadcast"));

        LocalBroadcastManager.getInstance(GuardianAddedService.this).registerReceiver(pauseJourneyActivity,
                new IntentFilter("pauseJourneyBroadcast"));
        LocalBroadcastManager.getInstance(GuardianAddedService.this).registerReceiver(playJourneyDBActivity,
    ...
    public BroadcastReceiver playJourneyDBActivity = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            alarmMgr = (AlarmManager)GuardianAddedService.this.getSystemService(Context.ALARM_SERVICE);
            Intent broadcastIntent = new Intent(GuardianAddedService.this, WardETABroadcastReceiver.class);
            broadcastIntent.putExtra("ward", userDetails.encodedEmail());
            alarmIntent = PendingIntent.getBroadcast(GuardianAddedService.this, 0, broadcastIntent, 0);
    ...
    public void onDestroy() {
        Log.i("GuardianAddedService", "Removing GuardianAddedService");
        super.onDestroy();
    }
}
